function About() {
  return (
    <section id="about" className="about-section">
      <div className="container">

        <div className="section-title">
          <p>About Me</p>
          <h2>Get to Know Me</h2>
        </div>

        <div className="about-content">

          <div className="about-text">

            <h3>Java Full Stack Developer</h3>

            <p>
              I am an Information Science graduate and Java Full Stack
              Developer with experience building web applications using
              React.js, Java, Spring Boot, REST APIs and SQL databases.
            </p>

            <p>
              I enjoy solving technical problems, developing responsive
              user interfaces and building reliable backend services.
            </p>

            <p>
              My experience includes working on dashboards, travel
              management platforms, learning systems and full-stack
              applications.
            </p>

          </div>

          <div className="about-highlights">

            <div className="highlight-card">
              <h3>8.67</h3>
              <p>Engineering CGPA</p>
            </div>

            <div className="highlight-card">
              <h3>Full Stack</h3>
              <p>Development Experience</p>
            </div>

            <div className="highlight-card">
              <h3>React + Java</h3>
              <p>Core Technology Stack</p>
            </div>

            <div className="highlight-card">
              <h3>REST APIs</h3>
              <p>Backend Development</p>
            </div>

          </div>

        </div>

      </div>
    </section>
  );
}

export default About;